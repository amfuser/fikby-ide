...
impl Editor {
    pub fn new(
        ...
        main_view.set_accepts_tab(false);
        ...
        line_numbers.set_width_request(55);
        ...
        tab_label.set_width_request(80);
        ...
        // Key handling
        {
            ...
            let settings_clone = editor.settings.clone();
            ...
            key_controller.connect_key_pressed(move |_, keyval, _keycode, modifier| {
                ...
                match keyval {
                    gdk::Key::Tab => {
                        if shift_pressed {
                            Self::decrease_indent(&buffer_clone, &settings_clone);
                        } else {
                            Self::increase_indent(&buffer_clone, &settings_clone);
                        }
                        Inhibit(true)
                    }
                    gdk::Key::Return | gdk::Key::KP_Enter => {
                        Self::auto_indent_newline(&buffer_clone, &settings_clone);
                        Inhibit(true)
                    }
                    gdk::Key::braceright => {
                        Self::handle_closing_bracket(&buffer_clone, &settings_clone, '}');
                        Inhibit(true)
                    }
        ...
        // Update line numbers when buffer changes
        {
            ...
            main_buffer.connect_changed(move |_| line_numbers_clone.queue_draw());
        }
        ...
        // Update line numbers when scrolling
        {
            ...
            vadj.connect_value_changed(move |_| line_numbers_clone.queue_draw());
        }

        // Dirty marker + tab label
        {
    ...
    fn indent_width(settings: &EditorSettings) -> usize {
        settings.indent_width().max(1) as usize
    }

    fn indent_unit(settings: &EditorSettings) -> String {
        match settings.indent_style() {
            IndentStyle::Spaces => " ".repeat(Self::indent_width(settings)),
            IndentStyle::Tabs => "\t".to_string(),
        }
    }

    fn increase_indent(buffer: &TextBuffer, settings: &EditorSettings) {
        let indent = Self::indent_unit(settings);

        let (has_selection, start, end) = buffer
            .selection_bounds()
            .map(|(s, e)| (true, s, e))
            .unwrap_or_else(|| {
        ...
        if has_selection {
            ...
            if end.line_offset() == 0 && end_line > start_line {
                end_line -= 1;
            }
    ...
    fn decrease_indent(buffer: &TextBuffer, settings: &EditorSettings) {
        let indent_width = Self::indent_width(settings);
        ...
        for line_num in start_line..=end_line {
            if let Some(mut line_start) = buffer.iter_at_line(line_num) {
                ...
                match settings.indent_style() {
                    ...
                    IndentStyle::Spaces => {
                        let mut spaces_to_remove = 0usize;
                        for ch in line_text.chars().take(indent_width) {
                            if ch == ' ' {
                                spaces_to_remove += 1;
                            } else {
                                break;
                        ...
                        if spaces_to_remove > 0 {
                            ...
                            end_of_spaces.forward_chars(spaces_to_remove as i32);
    ...
    fn auto_indent_newline(buffer: &TextBuffer, settings: &EditorSettings) {
        let indent_unit = Self::indent_unit(settings);
        ...
        if let Some(line_start) = buffer.iter_at_line(line) {
            let mut line_end = line_start.clone();
            line_end.forward_to_line_end();
            let full_line_text = buffer.text(&line_start, &line_end, false);

            let leading_ws: String = full_line_text
                .chars()
                .take_while(|c| *c == ' ' || *c == '\t')
                .collect();

            let before_cursor_text = buffer.text(&line_start, &iter, false);
            let trimmed = before_cursor_text.trim_end();
    ...
    fn handle_closing_bracket(buffer: &TextBuffer, settings: &EditorSettings, bracket: char) {
        let indent_width = Self::indent_width(settings);
        ...
        if let Some(mut line_start) = buffer.iter_at_line(line) {
            ...
            if text_before_cursor.trim().is_empty() {
                match settings.indent_style() {
                    ...
                    IndentStyle::Spaces => {
                        let leading_spaces =
                            text_before_cursor.chars().take_while(|c| *c == ' ').count();
                        if leading_spaces >= indent_width {
                            let mut delete_end = line_start.clone();
                            delete_end.forward_chars(indent_width as i32);
                            buffer.delete(&mut line_start, &mut delete_end);
                        }
                    }
                }
    ...
    fn update_status_bar(&self, status_label: &Label, status_info_label: &Label, content: &str) {
        ...
        let info = if let Some(p) = self.current_file.borrow().as_ref() {
            ...
            format!("Untitled — {} bytes", content.len())
        };
    ...
    fn trigger_highlighting(&self, content: &str) {
        let do_highlight = self
            ...
            .map(|ext| {
                matches!(
                    ext,
                    "rs" | "py" | "js" | "ts" | "json" | "toml" | "md" | "html" | "css" | "xml"
                )
            })
            .unwrap_or(true);
    ...
    // ----------------------------
    // Find / Replace (required by find_replace.rs)
    // ----------------------------
    pub fn find_text(&self, search_text: &str, case_sensitive: bool) -> bool {
        ...
        if let Some((mut match_start, match_end)) = start.forward_search(search_text, flags, None) {
            ...
            self.main_view
                .scroll_to_iter(&mut match_start, 0.0, false, 0.0, 0.0);
            true
        } else {
    ...
    pub fn replace_all(
        ...
        while let Some((mut match_start, mut match_end)) =
            search_start.forward_search(search_text, flags, None)
        {
